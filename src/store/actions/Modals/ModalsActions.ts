...
export const startToggleTextEditModal = () => {
    return (dispatch: Dispatch<AppActions>, getState: () => EntireAppState) => {
...
export const startTextEditModal = (actionType: string, index?: number) => {
    ...
    return (dispatch: Dispatch<AppActions>, getState: () => EntireAppState) => {
        const newModalState: TextEditModalState = TextEditModalStateSwitch(
            actionType,
            getState(),
            dispatch
        );
        console.debug(
            `newModalState in startTextEditModal: ${JSON.stringify(
                newModalState,
                null,
                1
            )}`
...
const TogglePickerModal: ActionCreator<ModalActionTypes> = (
    value: ReactText
): ModalActionTypes => ({
...
export const startNumberPickerModalSelection = (
    actionType: string,
    value: number
) => {
    return (dispatch: Dispatch<AppActions>, getState: () => EntireAppState) => {
        let newModalState: PickerModalState = PickerModalStateSwitch(
            actionType,
            getState(),
            dispatch
        );
...
export const startStringPickerModalSelection = (
    actionType: string,
    value: string
) => {
    return (dispatch: Dispatch<AppActions>, getState: () => EntireAppState) => {
...
export const startPickerForAbilityScore = (
    actionType: string,
