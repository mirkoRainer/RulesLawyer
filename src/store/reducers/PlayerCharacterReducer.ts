...
const playerCharacterReducer = (state=defaultState, action: PlayerCharacterActionTypes): PlayerCharacter => { 
    ...
    case CHANGE_HIT_POINTS:
        ...
        console.debug(`newHealthData: ${JSON.stringify(newHealthData, null, 1)}`);
        modifiedState = {
            ...state,
            hitPoint: newHealthData
        };
        console.debug(`result: ${JSON.stringify(modifiedState.hitPoint, null, 1)}`);
        return modifiedState;
    ...
    case CHANGE_SAVE_PROFICIENCIES:
        console.debug(`CHANGE_SAVE_PROFICIENCIES in reducer. ${JSON.stringify(action.saves, null, 1)}`);
        modifiedState = {
            ...state,
            saves: action.saves
        };
        return modifiedState;
    case CHANGE_SHIELD:
        console.debug(`SHIELD in reducer. ${JSON.stringify(action.Shield, null, 1)}`);
        modifiedState = {
            ...state,
    ...
    case CHANGE_WORN_ARMOR:
        console.debug(`CHANGE_WORN_ARMOR in reducer ${JSON.stringify(action.WornArmor, null, 1)}`);
        return {
            ...state,
            wornArmor: action.WornArmor
        };
    case CHANGE_PERCEPTION_PROFICIENCY:
        console.debug(`CHANGE_PERCEPTION_PROFICIENCY in reducer ${JSON.stringify(action, null, 1)}`);
        return {
            ...state,
            perceptionProficiency: action.PerceptionProficiency
        };
    case CHANGE_SPEED:
        console.debug(`CHANGE_SPEED in reducer ${JSON.stringify(action, null, 1)}`);
        return {
            ...state,
            movement: action.Movements
        };
    case CHANGE_PC_ACTIONS:
        console.debug(`CHANGE_PF2ACTIONS in reducer ${JSON.stringify(action, null, 1)}`);
        return {
            ...state,
            actions: action.Actions
        };
    case CHANGE_SKILLS:
        console.debug(`CHANGE_SKILLS in reducer ${JSON.stringify(action, null, 1)}`);
        return {
            ...state,
