...
const ShieldView: React.FC<Props> = (props) => {
    ...
    const adjustShieldHP = (amount: number) => {
        ...
        if (adjustedHP > props.shield.maxHP) {
            props.updateShield({...props.shield, currentHP: props.shield.maxHP});
            return;
        }
    ...
    const shieldData = () => {
        if (props.shield.currentHP > props.shield.breakThreshold) {
            ...
            return (
                <>
                    <Text category='h6' status='warning' style={{flex: 1, alignSelf: "stretch", textAlign: "center"}}>Broken</Text>
                </>
            );
        }
    };
    ...
    const shieldView = props.shield.hasShield ? (
        <Layout style={styles.container}>
            <Layout style={{flex: .25, justifyContent: "center"}}>
                <Text style={styles.title} category='h6'>Shield</Text>
                <Toggle checked={props.shield.hasShield} onChange={onShieldToggle}></Toggle>
            </Layout>
            <Layout style={styles.shield}>
                <Layout style={styles.horizontal}>
                    ...
                    <TouchableOpacity onPress={modalOn} style={styles.hpNumber}>
                        <Text category='h6' >HP: {props.shield.currentHP!}/{props.shield.maxHP!}</Text>
                    </TouchableOpacity>
                    <ButtonGroup style={styles.buttonGroup} size="tiny" status='info'>
                        {PlusHPButton(5)}
                        {PlusHPButton(1)}
                    </ButtonGroup>
                </Layout>
        ...
        <Layout style={{paddingVertical: 5}}>
            <Toggle checked={props.shield.hasShield} onChange={onShieldToggle}>{"Equip Shield"}</Toggle>
        </Layout>
    );
